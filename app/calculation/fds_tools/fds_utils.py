...
class FireTools:
    ...
    def compute_deltas(self, evac_times, density_threshold):
        ...
        for time, max_density in evac_times:
            if max_density >= density_threshold:
                ...
                # Тут к конечной сумме total_delta_sum прибавляется лишнее значение 0.2. Оригинал: "delta = time - evac_times[-1][0] if evac_times[-1][1] >= density_threshold else 0.2". В оригинале delta почему-то принимает отрицательное значение.
                delta = time - \
                    evac_times[-1][0] if evac_times[0][1] >= density_threshold else 0.2
            else:
                delta = 0
            deltas.append(delta)
    ...
    def process_data(self, file_paths, density_threshold=0.5):
        ...
        total_delta_sum, first_time_exceeded, last_time_exceeded = compute_deltas(
            evac_times, density_threshold)

        print(
            f"tск* - Время существования всех скоплений на временном отрезке [{first_time_exceeded} ; {last_time_exceeded}] сек, \nгде плотность потока, состоящего хотя бы из 2 человек, превышает {density_threshold} (м2/м2): {total_delta_sum:.1f} сек.")
    ...
    def plot_densities(evac_times, density_threshold, total_delta_sum, first_time_exceeded, last_time_exceeded):
        ...
        plt.axhline(density_threshold, color="red", linestyle="--", lw=2,
                    label=f'Критическая плотность >= {density_threshold} (м2/м2)')
