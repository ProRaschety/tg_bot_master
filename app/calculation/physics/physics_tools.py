...
class PhysicTool:
    ...
    def get_init_data(self, *args, **kwargs):
        ...
        if self.type_substance == 'liquid':
            ...
            data_table = [
                ...
                #     'tool_liquid_hole_distance'), 'unit_2': 'м'},
                # {'id': 'Высота резервуара', 'var': 'Hp',  'unit_1': kwargs.get(
                #     'tool_comp_gas_height_vessel'), 'unit_2': 'м'},
                {'id': 'Объем резервуара', 'var': 'Vр',
                    'unit_1': f"{float((kwargs.get('tool_comp_gas_volume_vessel', 1000))):.1f}", 'unit_2': 'м\u00B3'},
                {'id': 'Показатель адиабаты газа', 'var': 'ϒ',
                    'unit_1': 1.32, 'unit_2': '-'},
                {'id': 'Темепература газа в оборудовании', 'var': 'tг', 'unit_1': kwargs.get(
                    'tool_comp_gas_temperature'), 'unit_2': '\u00B0С'},
                {'id': 'Плотность газа в резервуаре\nпри начальном давлении',
                    'var': 'ρ', 'unit_1': f"{float(0.785):.3f}", 'unit_2': 'кг/м\u00B3'},
                {'id': 'Начальное давление газа в резервуаре',
                    'var': 'Pv', 'unit_1': f"{float(1600000):.2e}", 'unit_2': 'Па'},
                {'id': 'Атмосферное давление', 'var': 'Pa', 'unit_1': 101325, 'unit_2': 'Па'}]
    ...
    def compute_init_mass_flow_rate(self, **kwargs):
        ...
        temperature_liquid = float((kwargs.get('tool_liquid_temperature')))
        fill_factor = float((kwargs.get('tool_liquid_fill_factor')))
        hole_diameter = float((kwargs.get('tool_liquid_hole_diameter')))
        vessel_diameter = float((kwargs.get('tool_liquid_vessel_diameter')))
        ...
        initial_pressure_in_vessel = density_liq * 9.81 * \
            initial_height_liquid + p_atm  # Давление в оборудовании, Па
        # начальный массовый расход, кг/с
        initial_mass_flow = mu * hole_area * \
            m.sqrt(2 * (initial_pressure_in_vessel - p_atm) * density_liq)
    ...
    def _compute_mass_flow_rate(self, **kwargs):
        ...
        for t in range(0, max_range, 1):
            if t == 0:
                ...
                G_i = mu * hole_area * \
                    m.sqrt((2 * (p_equip - p_atm) * density_liq))
                ...
                if float(p_equip) > p_atm:
                    ...
                    velocity_outflow.append(m.sqrt(2 * 9.81 * h_liq_i))

                    # длина струи жидкости, м
                    lenght_jet = m.sqrt((2 * 9.81 * h_liq_i)
    ...
    def get_plot_mass_flow_rate(self, add_annotate: bool = False, add_legend: bool = False, **kwargs):
        log.info("График расхода жидкости")
        ...
        fig_ax_1.text(x=0.0, y=0.025, s='График расхода жидкости',
        ...
        if add_annotate:
            ...
            fig_ax_2.annotate(f"Начальный массовый расход: {max(plot_first):.2f} кг/с\n"
                              f"Начальный объемный расход: {(max(plot_first) / float(kwargs.get('tool_liquid_density'))):.4f} м\u00B3/с\n"
                              f"Время полного истечения: {max(abscissa):.1f} с\n"
                              f"Время полного истечения: {(max(abscissa)/60):.1f} мин\n",
                              xy=(0, 0), xycoords='data', xytext=(max(abscissa)*0.03, max(plot_first)*0.03), textcoords='data', weight='bold', **ft_size)
