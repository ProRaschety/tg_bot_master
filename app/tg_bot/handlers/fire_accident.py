...
async def fire_pool_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    accmodel = AccidentModel(substance_state='liquid',  # 'solid', 'liquid', 'gas', 'dust', 'liquid_gas', 'nonflammable'
                             substance_name='gasoline')  # 'gasoline', 'diesel', 'LNG', 'LPG', 'liq_hydrogen'
    substance = SubstanceModel(substance_name=accmodel.substance_name,
                               molar_mass=molar_mass,
                               boiling_point=boling_point,
                               mass_burning_rate=m)
...
async def fire_pool_subst_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    molar_mass, boling_point, m = await get_property_fuel(subst=call_data)
    ...
    accmodel = AccidentModel(**context_data.get('accmodel'))
...
async def edit_pool_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    accmodel = AccidentModel(**data.get('accmodel'))
...
async def edit_fire_pool_param_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    if user_state == FSMFireAccidentForm.edit_fire_pool_area_state:
        ...
        if value != '' and value != '.' and (float(value)) > 0:
            ...
            await state.update_data(accmodel.update(velocity_wind=float(value)))
        else:
...
async def run_fire_pool_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    nonvelocity = f_pool.compute_nonvelocity(
        wind=accmodel.velocity_wind, density_fuel=fuel_density, mass_burn_rate=mass_burn_rate, eff_diameter=diameter)
    ...
    x0 = misc_utils.get_distance_at_value(x_values=x, y_values=y, value=4.0)

    headers = (i18n.get('name'), i18n.get('variable'),
               i18n.get('value'), i18n.get('unit'))
    label = i18n.get('fire_pool')
    data_out = [
        {'id': i18n.get('distance_to_safe_zone_from_the_heat_flux'), 'var': 'x0',
            'unit_1': f"{x0:.2f}", 'unit_2': i18n.get('meter')},
...
async def plot_fire_pool_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, 'probit_fire_pool', i18n=i18n, param_back=True, back_data='back_fire_pool'))
    await callback.answer('')
...
async def probability_defeat_firepool_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        reply_markup=get_inline_cd_kb(
            1,
            'plot_fire_pool', i18n=i18n, param_back=True, back_data='back_fire_pool'
        )
    )
    await callback.answer('')
...
async def edit_cloud_explosion_guest_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_caption(
        ...
        reply_markup=get_inline_cd_kb(1,
                                      *i18n.get('cloud_explosion_kb_guest').split('\n'),
                                      i18n=i18n, param_back=True, back_data='back_cloud_explosion'))
    text = i18n.get('repeated_request_guest')
    await bot.edit_message_caption(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
...
async def cloud_explosion_methodology_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'methodology_404', 'methodology_2024', i18n=i18n, param_back=True, back_data='back_edit_cloud_explosion',))
...
@fire_accident_router.callback_query(F.data.in_(['edit_cloud_explosion_correction_parameter', 'edit_cloud_explosion_stc_coef_oxygen', 'edit_cloud_explosion_coef_z', 'edit_cloud_explosion_mass_fuel', 'edit_cloud_explosion_distance']))
...
async def run_cloud_explosion_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, 'plot_accident_cloud_explosion_pressure', 'plot_accident_cloud_explosion_impuls', i18n=i18n, param_back=True, back_data='back_cloud_explosion'))
    await callback.answer('')
...
async def horizontal_jet_plot_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(i18n=i18n, param_back=True, back_data='back_horizontal_jet'))
...
@fire_accident_router.callback_query(StateFilter(*SFilter_horizontal_jet), F.data.in_(['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'zero', 'dooble_zero', 'point', 'clear']))
async def edit__horizontal_jet_in_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    user_state = await state.get_state()
    if user_state == FSMFireAccidentForm.edit_horizontal_jet_mass_flow_state:
        hjet_param = i18n.get("name_hjet_mass_rate")

    elif user_state == FSMFireAccidentForm.edit_horizontal_jet_distance_state:
        hjet_param = i18n.get("name_hjet_distance")

    edit_data = await state.get_data()
    if callback.data == 'clear':
        await state.update_data(edit_accident_horizontal_jet_param="")
        edit_d = await state.get_data()
        edit_data = edit_d.get('edit_accident_horizontal_jet_param', 1)
        text = i18n.edit_hjet.text(
            hjet_param=hjet_param, edit_hjet=edit_data)

    else:
        edit_param_1 = edit_data.get('edit_accident_horizontal_jet_param')
        edit_sum = edit_param_1 + i18n.get(callback.data)
        await state.update_data(edit_accident_horizontal_jet_param=edit_sum)
        edit_data = await state.get_data()
        edit_param = edit_data.get('edit_accident_horizontal_jet_param', 0)
        text = i18n.edit_hjet.text(
            hjet_param=hjet_param, edit_hjet=edit_param)
...
@fire_accident_router.callback_query(StateFilter(*SFilter_horizontal_jet), F.data.in_(['ready']))
async def edit_hjet_param_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    user_state = await state.get_state()
    data = await state.get_data()
    value = data.get("edit_accident_horizontal_jet_param")

    if user_state == FSMFireAccidentForm.edit_horizontal_jet_mass_flow_state:
        if value != '' and value != '.' and (float(value)) > 0:
            await state.update_data(accident_horizontal_jet_mass_rate=value)
        else:
            await state.update_data(accident_horizontal_jet_mass_rate=1)
    elif user_state == FSMFireAccidentForm.edit_horizontal_jet_distance_state:
        if value != '' and value != '.' and (float(value)) > 0:
            await state.update_data(accident_horizontal_jet_human_distance=value)
        else:
            await state.update_data(accident_horizontal_jet_human_distance=10)
...
async def vertical_jet_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    data_out = [
        {'id': i18n.get('jet_human_distance'),
            'var': 'r',
            'unit_1': data.get('accident_vertical_jet_human_distance'),
            'unit_2': i18n.get('meter')},
        {'id': i18n.get('hjet_flame_width'),
            'var': 'Df',
            'unit_1': f'{diameter_flame:.2f}',
...
@fire_accident_router.callback_query(StateFilter(*SFilter_vertical_jet), F.data.in_(['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'zero', 'dooble_zero', 'point', 'clear']))
async def edit_vertical_jet_in_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    user_state = await state.get_state()
    if user_state == FSMFireAccidentForm.edit_vertical_jet_mass_flow_state:
        vjet_param = i18n.get("name_hjet_mass_rate")

    elif user_state == FSMFireAccidentForm.edit_vertical_jet_distance_state:
        vjet_param = i18n.get("name_hjet_distance")

    edit_data = await state.get_data()
    if callback.data == 'clear':
        await state.update_data(edit_accident_vertical_jet_param="")
        edit_d = await state.get_data()
        edit_data = edit_d.get('edit_accident_vertical_jet_param', 1)
        text = i18n.edit_vjet.text(
            vjet_param=vjet_param, edit_vjet=edit_data)

    else:
        edit_param_1 = edit_data.get('edit_accident_vertical_jet_param')
        edit_sum = edit_param_1 + i18n.get(callback.data)
        await state.update_data(edit_accident_vertical_jet_param=edit_sum)
        edit_data = await state.get_data()
        edit_param = edit_data.get('edit_accident_vertical_jet_param', 0)
        text = i18n.edit_vjet.text(
            vjet_param=vjet_param, edit_vjet=edit_param)
...
@fire_accident_router.callback_query(StateFilter(*SFilter_vertical_jet), F.data.in_(['ready']))
async def edit_vjet_param_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    user_state = await state.get_state()
    data = await state.get_data()
    value = data.get("edit_accident_vertical_jet_param")

    if user_state == FSMFireAccidentForm.edit_vertical_jet_mass_flow_state:
        if value != '' and value != '.' and (float(value)) > 0:
            await state.update_data(accident_vertical_jet_mass_rate=value)
        else:
            await state.update_data(accident_vertical_jet_mass_rate=1)
    elif user_state == FSMFireAccidentForm.edit_vertical_jet_distance_state:
        if value != '' and value != '.' and (float(value)) > 0:
            await state.update_data(accident_vertical_jet_human_distance=value)
        else:
            await state.update_data(accident_vertical_jet_human_distance=10)
...
async def fire_ball_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    data.setdefault("accident_fire_ball_distance", "30")
...
async def edit_fire_ball_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(4, *i18n.get('edit_fire_ball_kb').split('\n'),
...
async def run_fire_ball_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, 'plot_fire_ball', 'probit_fire_ball', i18n=i18n, param_back=True, back_data='back_fire_ball'))
    await callback.answer('')
...
async def plot_fire_ball_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, 'probit_fire_ball', i18n=i18n, param_back=True, back_data='back_fire_ball'))
    await callback.answer('')
...
async def probability_defeat_fireball_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, 'plot_fire_ball', i18n=i18n, param_back=True, back_data='back_fire_ball'))
    await callback.answer('')
...
async def edit_accident_bleve_guest_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_caption(
        ...
        reply_markup=get_inline_cd_kb(1,
                                      *i18n.get('accident_bleve_kb_guest').split('\n'),
                                      i18n=i18n, param_back=True, back_data='back_accident_bleve'))
    text = i18n.get('repeated_request_guest')
    await bot.edit_message_caption(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
...
async def edit_accident_bleve_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(4, *i18n.get('edit_accident_bleve_kb').split('\n'), i18n=i18n, param_back=True, back_data='back_accident_bleve'))
    await callback.answer('')
...
async def run_bleve_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, 'plot_accident_bleve_pressure', 'plot_accident_bleve_impuls', i18n=i18n, param_back=True, back_data='back_accident_bleve'))
...
async def plot_accident_bleve_pressure_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        reply_markup=get_inline_cd_kb(1,
                                      ...
                                      'plot_accident_bleve_impuls', i18n=i18n, param_back=True, back_data='back_accident_bleve'))
    await callback.answer('')
...
async def edit_bleve_param_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, *i18n.get('edit_accident_bleve_kb').split('\n'), i18n=i18n, param_back=True, back_data='back_accident_bleve'))
    await state.update_data(edit_accident_bleve_param='')
    await state.set_state(state=None)
