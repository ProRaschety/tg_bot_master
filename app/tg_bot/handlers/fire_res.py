...
async def back_steel_element_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    # chat_id = callback.message.chat.id
    text = i18n.type_of_calculation_steel.text()
    media = get_picture_filling(file_path='temp_files/temp/fsr_logo.png')
    await bot.edit_message_media(
...
async def thermal_calculation_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    data_out = [
        {'id': 'Коэффициент изм.\nтеплоемкости стали',
            ...
            'unit_2': i18n.get('J_per_kg_in_kelvin_square')},
        {'id': 'Теплоемкость стали',
         'var': 'Сст',
            'unit_1': t_res.heat_capacity,
            'unit_2': i18n.get('J_per_kg_in_kelvin')},
        ...
        {'id': 'Конвективный коэффициент\nтеплоотдачи',
            ...
            'unit_2': i18n.get('watt_per_meter_square_in_kelvin')},
    ...
    await bot.edit_message_media(
        ...
        reply_markup=get_inline_cd_kb(1, *i18n.get('thermal_calculation_kb_' + role).split('\n'),
...
async def ptm_edit_clear_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ptm_data = await state.get_data()
    await state.update_data(ptm="")
    ptm_data = await state.get_data()
    ptm_edit = ptm_data.get('ptm', 4.8)
