...
async def public_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1,
                                      *i18n.get('public_kb_guest').split('\n') if role in ['guest'] else i18n.get('public_kb').split('\n'),
                                      i18n=i18n, param_back=True, back_data='back_fire_risks_calc'))
...
async def run_public_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_caption(
        ...
        message_id=callback.message.message_id,
        caption=text,
        reply_markup=get_inline_cd_kb(i18n=i18n, param_back=True, back_data='back_public'))
...
@fire_risk_router.callback_query(F.data.in_(['edit_public_guest']))
async def edit_public_guest_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    await bot.edit_message_caption(
        ...
        reply_markup=get_inline_cd_kb(1,
                                      *i18n.get('public_kb_guest').split('\n'),
                                      i18n=i18n, param_back=True, back_data='back_public'))
...
async def k_efs_pub_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_efs_pub_true', 'k_efs_pub_false', i18n=i18n, param_back=True, back_data='stop_edit_public'))
    await callback.answer('')
...
async def k_alarm_pub_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_alarm_pub_true', 'k_alarm_pub_false', i18n=i18n, param_back=True, back_data='stop_edit_public'))
    await callback.answer('')
...
async def k_evacuation_pub_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_evacuation_pub_true', 'k_evacuation_pub_false', i18n=i18n, param_back=True, back_data='stop_edit_public'))
    await callback.answer('')
...
async def k_smoke_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_smoke_pub_true', 'k_smoke_pub_false', i18n=i18n, param_back=True, back_data='stop_edit_public'))
    await callback.answer('')
...
async def industrial_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    ...
    data.setdefault("probability_systems_effectiveness", "0")
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1,
                                      *i18n.get('industrial_kb_guest').split('\n') if role in ['guest'] else i18n.get('industrial_kb').split('\n'),
                                      i18n=i18n, param_back=True, back_data='back_fire_risks_calc'))
    await state.update_data(data)


@fire_risk_router.callback_query(F.data.in_(['edit_industrial_guest']))
async def edit_industrial_guest_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, role: UserRole) -> None:
    text = i18n.get('initial_request_guest')
    await bot.edit_message_caption(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
...
@fire_risk_router.callback_query(F.data.in_(['probability_systems_effectiveness']))
async def probability_systems_effectiveness_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    text = i18n.request_start.text()
    await bot.edit_message_caption(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        caption=text,
        reply_markup=get_inline_cd_kb(i18n=i18n, param_back=True, back_data='stop_edit_industrial'))

    text = i18n.probability_systems_effectiveness.text()
    media = get_picture_filling(
        file_path='temp_files/temp/pic_probability_systems_effectiveness.png')
    await bot.edit_message_media(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        media=InputMediaPhoto(media=BufferedInputFile(
        ...
        reply_markup=get_inline_cd_kb(5,
                                      *i18n.get('probability_systems_effectiveness_kb').split('\n'),
                                      i18n=i18n, param_back=True, back_data='stop_edit_industrial'))


@fire_risk_router.callback_query(F.data.in_(['probit_systems_effect_d0', 'probit_systems_effect_d1', 'probit_systems_effect_d2', 'probit_systems_effect_d3', 'probit_systems_effect_d4']))
async def probability_systems_effectiveness_call_in_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    text = i18n.request_start.text()
    await bot.edit_message_caption(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
    ...
    call_data = callback.data
    # data = await state.get_data()
    if call_data == 'probit_systems_effect_d0':
        await state.update_data(probability_systems_effectiveness=0)
    elif call_data == 'probit_systems_effect_d1':
        await state.update_data(probability_systems_effectiveness=1)
    elif call_data == 'probit_systems_effect_d2':
        await state.update_data(probability_systems_effectiveness=2)
    elif call_data == 'probit_systems_effect_d3':
        await state.update_data(probability_systems_effectiveness=3)
    elif call_data == 'probit_systems_effect_d4':
        await state.update_data(probability_systems_effectiveness=4)
...
async def k_efs_ind_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_efs_ind_true', 'k_efs_ind_true_095', 'k_efs_ind_false', i18n=i18n, param_back=True, back_data='stop_edit_industrial'))
    await callback.answer('')
...
async def k_alarm_ind_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_alarm_ind_true', 'k_alarm_ind_false', i18n=i18n, param_back=True, back_data='stop_edit_industrial'))
    await callback.answer('')
...
async def k_evacuation_ind_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_evacuation_ind_true', 'k_evacuation_ind_false', i18n=i18n, param_back=True, back_data='stop_edit_industrial'))
    await callback.answer('')
...
async def k_smoke_ind_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'k_smoke_ind_true', 'k_smoke_ind_false', i18n=i18n, param_back=True, back_data='stop_edit_industrial'))
    await callback.answer('')
...
async def emergency_escape_ind_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    await bot.edit_message_reply_markup(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
        reply_markup=get_inline_cd_kb(2, 'emergency_escape_ind_true', 'emergency_escape_ind_false', i18n=i18n, param_back=True, back_data='stop_edit_industrial'))
    await callback.answer('')
...
async def edit_fire_freq_ind_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    await bot.edit_message_caption(
        ...
        message_id=callback.message.message_id,
        caption=text,
        reply_markup=get_inline_cd_kb(i18n=i18n, param_back=True, back_data='table_404'))
...
async def edit_industrial_ready_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    text = i18n.request_start.text()
    await bot.edit_message_caption(
        chat_id=callback.message.chat.id,
        message_id=callback.message.message_id,
...
async def run_industrial_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, i18n=i18n, param_back=True, back_data='back_industrial'))
    # await state.update_data(data)
    await state.set_state(state=None)
    await callback.answer('')
