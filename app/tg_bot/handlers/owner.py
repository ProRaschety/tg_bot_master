...
@owner_router.callback_query(F.data.in_(['owner_panel', 'cansel_enter_promo']))
...
async def get_users_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, db: DB) -> None:
    media = get_picture_filling(file_path='temp_files/temp/fsr_logo.png')
...
async def clear_promocodes_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner, db: DB) -> None:
    media = get_picture_filling(file_path='temp_files/temp/fsr_logo.png')
...
async def set_promocode_call(callback: CallbackQuery, bot: Bot, state: FSMContext, i18n: TranslatorRunner) -> None:
    message_id = callback.message.message_id
...
async def promo_code_input(message: Message, bot: Bot, state: FSMContext, i18n: TranslatorRunner, db: DB) -> None:
    ...
    await bot.edit_message_media(
        ...
        media=InputMediaPhoto(media=BufferedInputFile(
            file=media, filename="pic_filling"), caption=text),
        reply_markup=get_inline_cd_kb(1, 'get_promocode', 'clear_promocodes', 'set_promocode', 'owner_panel', i18n=i18n))
